    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureAppConfiguration((_, configBuilder) =>
        {
            configBuilder.AddInMemoryCollection(new Dictionary<string, string?>
            {
                ...
                ["ConnectionStrings:Dashboard"] = "Host=localhost;Database=unused;Username=unused;Password=unused",
            });
        });
        ...
        builder.ConfigureServices(services =>
        {
            ...
            // DashboardDbContext with UseNpgsql by the time this runs. Removing
            // just the DbContextOptions<DashboardDbContext> descriptor isn't
            ...
            // DashboardDbContext -- by name or as a generic argument -- before
            // re-registering it fresh.
            var descriptorsToRemove = services
                .Where(d =>
                    d.ServiceType == typeof(DashboardDbContext) ||
                    (d.ServiceType.IsGenericType &&
                     d.ServiceType.GetGenericArguments().Contains(typeof(DashboardDbContext))))
                .ToList();
            ...
            services.AddDbContext<DashboardDbContext>(options => options.UseSqlite(_connection));
        });
    }

    /// <summary>Creates the schema from the current model and seeds it with the given data.</summary>
    public async Task InitializeDatabaseAsync(Action<DashboardDbContext> seed)
    {
